...
static rt_err_t rt_dc_init(rt_device_t dev)
{
    ...
    reg_write_32(0x0,(volatile unsigned int *)DC_FB_CONFIG1);
    //DC_FB_CONFIG = 0x0;
    reg_write_32(0x0,(volatile unsigned int *)DC_FB_CONFIG1);
    //DC_FB_CONFIG = 0x3; //    // framebuffer configuration RGB565
    reg_write_32(0x0,(volatile unsigned int *)DC_DITHER_CONFIG1);
    //DC_DITHER_CONFIG = 0x0;  //颜色抖动配置寄存器
    reg_write_32(0x0,(volatile unsigned int *)DC_DITHER_TABLE_LOW1);
    //DC_DITHER_TABLE_LOW = 0x0; //颜色抖动查找表低位寄存器 
    reg_write_32(0x0,(volatile unsigned int *)DC_DITHER_TABLE_HIGH1);
    //DC_DITHER_TABLE_HIGH = 0x0; //颜色抖动查找表高位寄存器
    reg_write_32(0x0,(volatile unsigned int *)DC_PANEL_CONFIG1);
    //DC_PANEL_CONFIG = 0x80001311; //液晶面板配置寄存器
    reg_write_32(0x0,(volatile unsigned int *) DC_PANEL_TIMING1);
    //DC_PANEL_TIMING = 0x0;
    reg_write_32((vga_mode[mode].hfl<<16) | vga_mode[mode].hr,(volatile unsigned int *)DC_HDISPLAY1);
    //DC_HDISPLAY = (vga_mode[mode].hfl<<16) | vga_mode[mode].hr;
    reg_write_32((0x40000000 | (vga_mode[mode].hse<<16) | vga_mode[mode].hss),(volatile unsigned int *)DC_HSYNC1);
    //DC_HSYNC = 0x40000000 | (vga_mode[mode].hse<<16) | vga_mode[mode].hss;
    reg_write_32(((vga_mode[mode].vfl<<16) | vga_mode[mode].vr),(volatile unsigned int *)DC_VDISPLAY1);
    //DC_VDISPLAY = (vga_mode[mode].vfl<<16) | vga_mode[mode].vr;
    reg_write_32((0x40000000 | (vga_mode[mode].vse<<16) | vga_mode[mode].vss),(volatile unsigned int *)DC_VSYNC1);
    ...
    reg_write_32(0x00100103,(volatile unsigned int *)DC_FB_CONFIG1);
    //DC_FB_CONFIG = 0x00100103;
    reg_write_32((FB_XSIZE*2+0x7f)&(~0x7f),(volatile unsigned int *)DC_FB_BUFFER_STRIDE1);
...
static rt_err_t rt_dc_control(rt_device_t dev, int cmd, void *args)
{
    ...
    case RTGRAPHIC_CTRL_RECT_UPDATE:
    {
        if (fb_index == 0)
        {   
            reg_write_32((rt_uint32_t)_rt_framebuffer1 - 0x80000000,(volatile unsigned int *)DC_FB_BUFFER_ADDR01);
            //DC_FB_BUFFER_ADDR0 = (rt_uint32_t)_rt_framebuffer1 - 0x80000000;
            reg_write_32((rt_uint32_t)_rt_framebuffer1 - 0x80000000,(volatile unsigned int *)DC_FB_BUFFER_ADDR11);
        ...
        {
            reg_write_32((rt_uint32_t)_rt_framebuffer0 - 0x80000000,(volatile unsigned int *)DC_FB_BUFFER_ADDR01);
