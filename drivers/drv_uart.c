...
void rt_hw_uart_init(void)
{
    ...
    serial9.ops    = &ls1c_uart_ops;
    serial9.config = config;

    pin_set_purpose(85, PIN_PURPOSE_OTHER);
    pin_set_purpose(86, PIN_PURPOSE_OTHER);
    pin_set_remap(85, PIN_REMAP_FIFTH);
    pin_set_remap(86, PIN_REMAP_FIFTH);
    

    rt_hw_interrupt_install(uart->IRQ, uart_irq_handler, &serial9, "UART9");

    /* register UART9 device */
    rt_hw_serial_register(&serial9,
                          "uart9",
                          //RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX | RT_DEVICE_FLAG_DMA_RX,
                          RT_DEVICE_FLAG_RDWR | RT_DEVICE_FLAG_INT_RX,
                          uart);
